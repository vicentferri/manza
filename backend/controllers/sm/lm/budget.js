...
function Add_Tipo_1(idrow,values){
  ...
  request.input('idcajon',sql.Int,idcajon);
   request.input('idguia',sql.Int,idguia);
     request.execute('sp_sol_presupuestos_cola_tipo_1_add', 
...
function Add_Tipo_2(idrow,values){
    ...
    var PJ_AltoLamaTerminada = 0;
        ...
        request.execute('sol_presupuestos_cola_tipo_2_add', 
        function(err, recordsets, returnValue) {
...
var request = new sql.Request();
  ...
  request.execute('sol_presupuestos_cola_tipo_3_add', 
        function(err, recordsets, returnValue) {
...
function Add_Tipo_4(idrow,values){
  ...
  request.input('Estancia_ID',sql.Int,Estancia_ID);
  request.input('Estancia',sql.VarChar(250),Estancia);
    request.execute('sol_presupuestos_cola_tipo_4_add', 
        function(err, recordsets, returnValue) {
 ...
 function budget(req,res)
 {
    ...
    var query = "select idrow,cliente,fecha,referencia,estado,refgeneral,dias,articulo,ancho,alto,cantidad,tipo,TipoArticulo,nomcliente,fec_validez from vw_cola_presupuestos";
...
function budget_hinzu(req,res)
{
   ...
   var request = new sql.Request();
    ...
    request.execute('sp_presupuestos_cola_add', 
    function(err, recordsets, returnValue) {
 ...
 function budget_hinzu2(req,res)
{
 ...
 function budget_herunterladen_T1(req,res)
 {
...
function budget_herunterladen_T2(req,res)
 {
 	var idrow = req.params.idrow;
 ...
 function budget_herunterladen_T3(req,res)
 {
 	var idrow = req.params.idrow;
...
function budget_herunterladen_T4(req,res)
 {
 	var idrow = req.params.idrow;
...
function budget_entrega_post(req,res)
{
...
function budget_image(req,res){

    var idrow = req.params.idrow;

    var path_file = 'uploads/budget_pdf/' + idrow + '.pdf';
    ...
    fs.exists(path_file, function(exists){
            ...
            if (exists){
               console.log("SEND FILE=" + path_file);
 ...
 function budget_image2(req,res){
 ...
 function budget_delete(req,res)
 {
	 	var idrow    = req.params.idrow;

        var request = new sql.Request();
        request.input('id',sql.Int,idrow);
        request.execute('sp_presupuesto_delete', 
        function(err, recordsets, returnValue) {
...
module.exports = {
    ...
    budget_image,
    budget_image2,
    budget_herunterladen_T1,
    budget_herunterladen_T2,
    budget_herunterladen_T3,
    budget_herunterladen_T4,
    budget_delete
}